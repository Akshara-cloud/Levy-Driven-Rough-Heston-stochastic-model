...
# price call options on the grid and calculate implied vols in-memory
def run_pricing_and_iv():
    model_simulators = {
    ...
    rows = []
    for model_name, sim_fn in model_simulators.items():
        print(f"Pricing model in-memory: {model_name}")
...
def plot_smile(rows, T_target=0.25, out_path="plots/implied_vol_smile.png"):
    ...
    for model_name, label in labels.items():
        model_data = [row for row in sub if row["model"] == model_name]
        model_data.sort(key=lambda x: x["K_over_S"])
        
        x = [row["K_over_S"] for row in model_data]
        y = [row["implied_vol"] for row in model_data]
...
if __name__ == "__main__":
    # check if implied vol inversion is correct
    known_sigma = 0.25
    known_price = bs_call(known_sigma, 100.0, 100.0, 0.5, r)
    recovered = implied_vol(known_price, 100.0, 100.0, 0.5, r)
    print(f"Inversion sanity check: input={known_sigma}, recovered={recovered:.4f}")

    # run pricing and calculate implied vols in-memory
    rows = run_pricing_and_iv()
    
    # plot volatility smile at T = 0.25
    plot_smile(rows, T_target=0.25)
